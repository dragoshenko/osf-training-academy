...
server.append('Show', function (req, res, next) {
    var viewData = res.getViewData(); // Get the existing view data

    // Check if the product has a sale price and calculate the discount percentage
    if (viewData.product && viewData.product.price && viewData.product.price.sales && viewData.product.price.sales.value) {
        var discountPercentage = productHelpers.calculatePercentageOff(viewData.product.price.list.value, viewData.product.price.sales.value);
        viewData.discountPercentage = discountPercentage; // Add the discount percentage to the view data
    }

    res.setViewData(viewData); // Update the view data
